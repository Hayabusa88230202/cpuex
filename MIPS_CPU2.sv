...
// =================================================================
    ...
    //wire mem_busy = ((mem_re && (data_addr != 32'hffff0000)) || (mem_we && (data_addr != 32'hffff0000))) && !mem_ready;
    // ★修正: UARTであってもDRAMであっても、!mem_ready なら絶対にストールする！
  ...
  /*  assign stall_pc     = load_use_stall || mem_busy || stall_fpu || structural_hazard;
    ...
    // ★修正: load_use_stall を global_load_use_stall に変更
    ...
    // ★修正: load_use_flush を global_load_use_flush に変更
...
// =================================================================
    ...
    InstructionMemory_readmemh instr_mem (
        ...
        .write_enable(((data_addr < 32'h00040000) && mem_we))
    );
...
// =================================================================
    ...
    // --- FPU ---
    // 1. MTC1 と FLW の命令判定 (EXステージ)
    wire is_mtc1_ex = (instruction_ex[31:26] == 6'h11) && (instruction_ex[25:21] == 5'd04);
    wire is_flw_ex  = (instruction_ex[31:26] == 6'h31);
    wire [4:0] flw_addr_ex = instruction_ex[20:16]; // flwの書き込み先は ft (20:16)

    // 2. FLW用のパイプライン遅延レジスタ (EX -> MEM -> WB)
    // flwのデータはWBステージでDRAMから上がってくるため、判定信号とアドレスをWBステージまで運ぶ
    logic       is_flw_mem, is_flw_wb;
    logic [4:0] flw_addr_mem, flw_addr_wb;
    ...
    always_ff @(posedge clk) begin
        if (!rst) begin
            ...
            if (!stall_ex_mem) begin
                is_flw_mem   <= is_flw_ex;
                flw_addr_mem <= flw_addr_ex;
            end
            if (!stall_mem_wb) begin
                is_flw_wb    <= is_flw_mem;
                flw_addr_wb  <= flw_addr_mem;
            end
        end
    end

    // =================================================================
    // ★★★ここに追加: FPU用ハザード（追い越し）検知回路★★★
    // =================================================================
    wire is_cop1_id  = (instruction_id[31:26] == 6'h11);
    wire is_fsw_id   = (instruction_id[31:26] == 6'h39);
    wire is_fmt_s_id = is_cop1_id && (instruction_id[25:21] == 5'd16);
    wire is_fmt_w_id = is_cop1_id && (instruction_id[25:21] == 5'd20); // ★追加: itof用 (fmt=20)
    wire is_mfc1_id  = is_cop1_id && (instruction_id[25:21] == 5'd00);

    // 次の命令(IDステージ)が、FPUレジスタ(fs, ft)を読む命令か判定
    wire id_reads_fpu_fs = is_fmt_s_id || is_fmt_w_id || is_mfc1_id; // ★修正: is_fmt_w_id を追加！
    wire id_reads_fpu_ft = is_fmt_s_id || is_fsw_id;  
    
    wire [4:0] id_fs_addr = instruction_id[15:11];
    wire [4:0] id_ft_addr = instruction_id[20:16];

    // EXステージまたはMEMステージにいる `flw` の書き込み先と、
    // 次に読もうとしているレジスタが被っていたらストール(一時停止)させる！
    wire fpu_hazard_ex = is_flw_ex && (
        (id_reads_fpu_fs && (flw_addr_ex == id_fs_addr)) ||
        (id_reads_fpu_ft && (flw_addr_ex == id_ft_addr))
    );
    wire fpu_hazard_mem = is_flw_mem && (
        (id_reads_fpu_fs && (flw_addr_mem == id_fs_addr)) ||
        (id_reads_fpu_ft && (flw_addr_mem == id_ft_addr))
    );
    assign fpu_load_use_stall = fpu_hazard_ex || fpu_hazard_mem;
    // =================================================================

    // 3. 外部書き込みポートのMUX
    // MTC1はEXステージで即座に書き込み、FLWはデータが届くWBステージで書き込む
    wire        fpu_ext_we    = is_mtc1_ex || is_flw_wb;
    wire [4:0]  fpu_ext_waddr = is_flw_wb ? flw_addr_wb : instruction_ex[15:11]; // flw優先
    //wire [31:0] fpu_ext_wdata = is_flw_wb ? Mem_ReadData_wb : ReadData2_ex;      // flwならメモリデータ、mtc1ならGPRデータ
    // mtc1のデータを、古いReadData2_exではなく、フォワーディング済みの最新データにする！
    ...
    FPU_Top fpu_unit (
        ...
        .ext_we(fpu_ext_we),
        .ext_waddr(fpu_ext_waddr),
        .ext_wdata(fpu_ext_wdata),
    ...
    // --- Branch & Jump Logic ---
    // ★ここを追加！！（1ビットへの切り捨てを阻止する）
    wire [31:0] branch_target_addr_ex;
    wire [31:0] jump_target_addr_ex;
...
// =================================================================
    ...
    assign inst_re   = (pc_current >= 32'h00040000) && rst;

    // --- Arbiter ---
    //assign mem_addr = data_addr;
    // データアクセス(is_mem_access)がある場合はそちらを優先。
    // 無い場合で、PCがDRAM領域(0x00400000以上)なら、命令フェッチとしてpc_currentを出力する！
    assign mem_addr = (is_mem_access) ? data_addr : 
                      (pc_current >= 32'h00040000) ? pc_current : 32'b0;
    assign mem_wdata = data_wdata;
    
    // ★修正: パルス回路を完全に撤去し、そのまま出力する！
    assign mem_we = data_we && ((data_addr >= 32'h00040000));

    //assign mem_re = data_re && ((data_addr >= 32'h00400000));
    // 読み出し要求 (データのロード、または、DRAMからの命令フェッチの時に1にする)
    assign mem_re = (data_re && (data_addr >= 32'h00040000)) || 
                    (!is_mem_access && (pc_current >= 32'h00040000));
