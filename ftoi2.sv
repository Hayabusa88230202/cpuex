...
module ftoi (
        ...
        if (!rst_n) begin
            ...
            if (!s1_use_fadd_reg) begin
               ...
               // if (shift_amt >= 31) shifted = {1'b1, mant} << shift_amt;
                ...
                // ★修正後：幅を明示して安全にシフト
                shifted = {8'b0, 1'b1, mant} << shift_amt;
    ...
    // ★修正後: input_valid のタイミングで確実に符号をキャッチして保持する
    logic saved_sign;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) saved_sign <= 1'b0;
        else if (input_valid) saved_sign <= in_f[31];
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (valid_fadd) begin
                ...
                // ★修正: sign_delay_chain[2] の代わりに saved_sign を使う
                if (saved_sign) begin
                    out_i <= (~abs_val + 1);
                end else begin
                    out_i <= abs_val;
